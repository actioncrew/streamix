...
export const coroutine = (...functions: Function[]): Coroutine => {
  ...
  const initWorkers = () => {
    ...
    const workerBody = `
    ${asyncPresent ?
      `import * as helpers from "https://unpkg.com/@actioncrew/streamix@1.0.9/fesm2022/actioncrew-streamix-coroutine-async.mjs";
  ...
  `;
    ...
    for (let i = 0; i < maxWorkers; i++) {
      workerPool.push(new Worker(workerUrl, {type: 'module'}));
    }
  };
