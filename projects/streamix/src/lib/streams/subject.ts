...
export function createSubject<T = any>(): Subject<T> {
  ...
  stream.complete = async function (this: Subject): Promise<void> {
    if (this[flags].isRunning && !this[internals].shouldComplete()) {
      ...
      return this[hooks].onComplete.waitForCompletion();
    }
  };
  ...
  stream.subscribe = (callbackOrReceiver?: ((value: T) => void) | Receiver<T>): Subscription => {
    ...
    if (receiver.complete) {
      stream[hooks].onComplete.chain(receiver, receiver.complete);
    }
    ...
    subscription.unsubscribe = () => {
      if (!subscription.unsubscribed) {
        ...
        const cleanup = () => {
          if (receiver.complete) stream[hooks].onComplete.remove(receiver, receiver.complete);
        ...
        if (!stream[flags].isStopped) {
          stream.complete().then(() => stream[hooks].onComplete.waitForCompletion()).then(cleanup);
        } else {
          cleanup();
        }
      }
